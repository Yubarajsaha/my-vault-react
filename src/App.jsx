...
const NO_PW_KEY = 'vault_no_pw_data'
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (settings.hasPassword || settings.hasBiometric) {
      ...
      setUser(u)
      loadNoPasswordVault(u)
    }
  }, [])

  async function loadNoPasswordVault(u) {
    try {
  ...
  async function persistVault(pw, data) {
    if (pw) {
      await saveVault(pw, data)
    } else {
      localStorage.setItem(NO_PW_KEY, JSON.stringify(data))
    }
  }
  ...
  function handleLock() {
    ...
    setProfile({}); setActivity([])
    setTab('home')
    if (settings.hasPassword || settings.hasBiometric) {
      setAuthState('login')
    } else {
      loadNoPasswordVault(user)
    }
  }

  async function handlePasswordSet(newPw) {
    const data = { docs, photos, secrets, profile, activity }
...
function SideBtn({ item, active, onClick }) {
